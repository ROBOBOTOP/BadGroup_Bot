...
basicConfig(
    format="%(asctime)s - [BAD] - %(levelname)s - %(message)s",
    level=INFO,
...
try:
    if environ.get("ENV"):
        ...
        from BADMUNDA.vars import Development as Config
...
LOGGER.info("------------------------")
LOGGER.info("| ⎯꯭̽🇨🇦꯭꯭ ⃪В꯭α꯭∂ ꯭м꯭υ꯭η∂꯭α_꯭آآ⎯꯭ ꯭̽🌸|")
LOGGER.info("------------------------")
...
defult_dev = [6898413162, 7032291324, 6352107773] + [int(OWNER_ID)]
...
async def load_cmds(all_plugins):
    ...
    for single in all_plugins:
        ...
        imported_module = imp_mod(f"BADMUNDA.plugins.{single}")
