...
@BAD.on_message(command("chatbroadcast", dev_cmd=True))
async def chat_broadcast(c: BAD, m: Message):
    if m.reply_to_message:
...
@BAD.on_message(command(["forward","fwd"],dev_cmd=True))
async def forward_type_broadcast(c: BAD, m: Message):
...
@BAD.on_message(command(["cleandb","cleandatabase"],sudo_cmd=True))
async def cleeeen(c:BAD,m:Message):
