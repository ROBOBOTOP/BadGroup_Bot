...
@BAD.on_message(command("unban") & restrict_filter)
async def unban_usr(c: BAD, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
...
@BAD.on_message(command("sban") & restrict_filter)
async def sban_usr(c: BAD, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(text="I can't ban nothing!")
        await m.stop_propagation()
...
@BAD.on_message(command("dban") & restrict_filter)
async def dban_usr(c: BAD, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(text="I can't ban nothing!")
        await m.stop_propagation()
...
@BAD.on_message(command("ban") & restrict_filter)
async def ban_usr(c: BAD, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(text="I can't ban nothing!")
        await m.stop_propagation()
...
@BAD.on_callback_query(regex("^unban_"))
async def unbanbutton(c: BAD, q: CallbackQuery):
...
@BAD.on_message(command("kickme"))
async def kickme(c: BAD, m: Message):
    reason = None
    if len(m.text.split()) >= 2:
        reason = m.text.split(None, 1)[1]
    try:
