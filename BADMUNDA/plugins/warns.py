...
from BADMUNDA import LOGGER, TIME_ZONE
...
from BADMUNDA.utils.custom_filters import admin_filter, command, restrict_filter
...
async def warn(c: BAD, m: Message):
...
@BAD.on_message(command("resetwarns") & restrict_filter)
async def reset_warn(c: BAD, m: Message):
...
@BAD.on_message(command("warns") & filters.group)
async def list_warns(c: BAD, m: Message):
...
async def remove_warn(c: BAD, m: Message):
...
@BAD.on_callback_query(filters.regex("^warn."))
async def remove_last_warn_btn(c: BAD, q: CallbackQuery):
...
@BAD.on_message(command(["warnings", "warnsettings"]) & admin_filter)
...
@BAD.on_message(command("warnmode") & admin_filter)
...
@BAD.on_message(command("warnlimit") & admin_filter)
...
Aᴅᴍɪɴ ᴄᴏᴍᴍᴀɴᴅs:
• /ᴡᴀʀɴ <ʀᴇᴀsᴏɴ>: Wᴀʀɴ ᴀ ᴜsᴇʀ.
• /ᴅᴡᴀʀɴ <ʀᴇᴀsᴏɴ>: Wᴀʀɴ ᴀ ᴜsᴇʀ ʙʏ ʀᴇᴘʟʏ, ᴀɴᴅ ᴅᴇʟᴇᴛᴇ ᴛʜᴇɪʀ ᴍᴇssᴀɢᴇ.
• /sᴡᴀʀɴ <ʀᴇᴀsᴏɴ>: Sɪʟᴇɴᴛʟʏ ᴡᴀʀɴ ᴀ ᴜsᴇʀ, ᴀɴᴅ ᴅᴇʟᴇᴛᴇ ʏᴏᴜʀ ᴍᴇssᴀɢᴇ.
• /ᴡᴀʀɴs: Sᴇᴇ ᴀ ᴜsᴇʀ's ᴡᴀʀɴɪɴɢs.
• /ʀᴍᴡᴀʀɴ: Rᴇᴍᴏᴠᴇ ᴀ ᴜsᴇʀ's ʟᴀᴛᴇsᴛ ᴡᴀʀɴɪɴɢ.
• /ʀᴇsᴇᴛᴡᴀʀɴ: Rᴇsᴇᴛ ᴀʟʟ ᴏғ ᴀ ᴜsᴇʀ's ᴡᴀʀɴɪɴɢs ᴛᴏ 0.
• /ᴡᴀʀɴɪɴɢs: Gᴇᴛ ᴛʜᴇ ᴄʜᴀᴛ's ᴡᴀʀɴɪɴɢ sᴇᴛᴛɪɴɢs.
• /ᴡᴀʀɴᴍᴏᴅᴇ <ʙᴀɴ/ᴋɪᴄᴋ/ᴍᴜᴛᴇ>: Sᴇᴛ ᴛʜᴇ ᴄʜᴀᴛ's ᴡᴀʀɴ ᴍᴏᴅᴇ.
• /ᴡᴀʀɴʟɪᴍɪᴛ <ɴᴜᴍʙᴇʀ>: Sᴇᴛ ᴛʜᴇ ɴᴜᴍʙᴇʀ ᴏғ ᴡᴀʀɴɪɴɢs ʙᴇғᴏʀᴇ ᴜsᴇʀs ᴀʀᴇ ᴘᴜɴɪsʜᴇᴅ.

Exᴀᴍᴘʟᴇs:
/ᴡᴀʀɴ @ᴜsᴇʀ: ᴛʜɪs ᴡᴀʀɴs ᴀ ᴜsᴇʀ ɪɴ ᴛʜᴇ ᴄʜᴀᴛ."""