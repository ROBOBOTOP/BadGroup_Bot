...
from BADMUNDA import LOGGER, MESSAGE_DUMP, SUPPORT_GROUP, TIME_ZONE
...
@BAD.on_message(command(["gban", "globalban"], sudo_cmd=True))
async def gban(c: BAD, m: Message):
...
async def ungban(c: BAD, m: Message):
    if len(m.text.split()) == 1:
