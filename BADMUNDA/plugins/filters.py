...
from BADMUNDA.bot_class import LOGGER, BAD
from BADMUNDA.database.filters_db import Filters
from BADMUNDA.utils.cmd_senders import send_cmd
from BADMUNDA.utils.custom_filters import admin_filter, command, owner_filter
from BADMUNDA.utils.kbhelpers import ikb
from BADMUNDA.utils.msg_types import Types, get_filter_type
from BADMUNDA.utils.regex_utils import regex_searcher
...
@BAD.on_message(command("filters") & filters.group & ~filters.bot)
...
@BAD.on_message(command(["filter", "addfilter"]) & admin_filter & ~filters.bot)
...
@BAD.on_message(command(["stop", "unfilter"]) & admin_filter & ~filters.bot)
...
@BAD.on_callback_query(filters.regex("^rm_allfilters$"))
...
async def send_filter_reply(c: BAD, m: Message, trigger: str):
...
@BAD.on_message(filters.text & filters.group & ~filters.bot, group=69)
async def filters_watcher(c: BAD, m: Message):
