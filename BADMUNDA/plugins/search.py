...
from BADMUNDA import LOGGER, SUPPORT_CHANNEL
...
@BAD.on_message(command('google'))
async def g_search(c: BAD, m: Message):
...
@BAD.on_message(command('anime'))
async def anime_search(c: BAD, m: Message):
    split = m.text.split(None, 1)
    if len(split) == 1:
        return await m.reply_text("No query given\nDo `/help search` to see how to use it")
...
@BAD.on_message(command('stack'))
async def stack_search(c: BAD, m: Message):
    split = m.text.split(None, 1)
    if len(split) == 1:
        return await m.reply_text("No query given\nDo `/help search` to see how to use it")
...
@BAD.on_message(command(["images","imgs"]))
