...
from BADMUNDA import LOGGER, SUPPORT_GROUP
...
from BADMUNDA.utils.extras import BAN_GIFS, KICK_GIFS, MUTE_GIFS
...
@BAD.on_message(command(['floodaction','actionflood']) & admin_filter)
async def flood_action(c: BAD, m: Message):
    Flood = Floods()
...
@BAD.on_message(command(['isflood', 'flood']) & ~filters.bot)
async def flood_on_off(c: BAD, m: Message):
    if m.chat.type == CT.PRIVATE:   
...
@BAD.on_message(command(['setflood']) & ~filters.bot & admin_filter)
async def flood_set(c: BAD, m: Message):
    bot = await c.get_chat_member(m.chat.id, Config.BOT_ID)
    Flood = Floods()
    status = bot.status
    if not status in [CMS.OWNER, CMS.ADMINISTRATOR]:
...
@BAD.on_callback_query(filters.regex("^f_"))
async def callbacks(c: BAD, q: CallbackQuery):
    data = q.data
    if data == "f_close":
...
@BAD.on_callback_query(filters.regex("^un_"))
async def reverse_callbacks(c: BAD, q: CallbackQuery):
    data = q.data.split("_")
    action = data[1]
...
dic = {}
@BAD.on_message(filters.all & ~filters.bot | ~filters.private, 10)
async def flood_watcher(c: BAD, m: Message):
    c_id = m.chat.id
