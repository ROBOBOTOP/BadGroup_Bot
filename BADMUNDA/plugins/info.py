...
from BADMUNDA import LOGGER, OWNER_ID
...
async def user_info(c: BAD, user, already=False):
    if not already:
    ...
    caption = f"""
<b><i><u>❤️ Exᴛʀᴀᴄᴛᴇᴅ Usᴇʀ ɪɴғᴏ Fʀᴏᴍ Tᴇʟᴇɢʀᴀᴍ ❤️ </b></i></u>

<b>❍ ᴜsᴇʀ ɪᴅ </b>: <code>{user_id}</code>
<b>❍ ʟɪɴᴋ ᴛᴏ ᴘʀᴏғɪʟᴇ </b>: <a href='tg://user?id={user_id}'>Click Here🚪</a>
<b>❍ ᴍᴇɴᴛɪᴏɴ </b>: {mention}
...
<b>❍ ɢʙᴀɴɴᴇᴅ</b>: {gban}
<b>❍ ɢʙᴀɴ ʀᴇᴀsᴏɴ</b>: <code>{reason}</code>
<b>❍ DC ID ᴅᴄ ɪᴅ </b>: {dc_id}
...
async def chat_info(c: BAD, chat, already=False):
    u_name = False
    if not already:
        try:
...
@BAD.on_message(command(["info", "whois"]))
async def info_func(c: BAD, message: Message):
...
@BAD.on_message(command(["chinfo", "chatinfo", "chat_info"]))
async def chat_info_func(c: BAD, message: Message):
